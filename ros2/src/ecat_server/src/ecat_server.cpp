  ...
  std::shared_ptr<ecat_interfaces::srv::NetworkCtrl::Response> response)
{
  ...
  switch (request->action) {
    case NetworkCtrl::Request::START:
      ...
      _ecat_master = std::make_unique<master_node>(_network_interface_param);
      _ecat_master->async_spin();
      _network_active.exchange(true);
      break;
    ...
    case NetworkCtrl::Request::STOP:
      ...
      _ecat_master->stop();
      _ecat_master = nullptr;
      _network_active.exchange(false);
      break;
  }
}
 ...
 **/
void Server::_handle_srv_set_parameters(
  const std::shared_ptr<rmw_request_id_t> request_header,
...
{
  if (!_network_active) {
    const char * msg = "Set parameters request rejected, network is not active";
 ...
 **/
void Server::_handle_srv_get_parameters(
  const std::shared_ptr<rmw_request_id_t> request_header,
...
{
  if (!_network_active) {
    const char * msg = "Get parameters request rejected, network is not active";
    RCLCPP_WARN(get_logger(), msg);
    response->msg = msg;
    response->success = false;
    return;
...
void Server::_execute_move(const std::shared_ptr<ExecuteMoveGoalHandle> goal_handle)
{
  ...
  const uint64_t fb_array_max = goal->feedback_chunk_size;
  ...
  for (std::size_t i = 0; (i < time_vec.size()) && rclcpp::ok(); i++) {
    ...
    if (time_fb.size() >= fb_array_max) {
      // Publish feedback when we reach the chunking size
  ...
  if (time_fb.size()) {
    // Publish whatever we have left over
    goal_handle->publish_feedback(feedback);
    //    std::string data{};
    //    for (std::size_t i = 0; i < time_fb.size(); i++) {
    //      data += std::to_string(counter) + ": (" + std::to_string(time_fb.at(i)) + ", " +
