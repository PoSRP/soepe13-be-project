  ...
  const rclcpp_action::GoalUUID & uuid, std::shared_ptr<const ExecuteMove::Goal> goal)
{
  ...
  std::string data{};
  for (std::size_t i = 0; i < goal->time_data.size(); i++) {
    data += "(" + std::to_string(goal->time_data.at(i)) + ", " +
      std::to_string(goal->value_data.at(i)) + ")";
    if (i < goal->time_data.size() - 1) {
      data += "; ";
    }
  }
...
void Server::_execute_move(const std::shared_ptr<ExecuteMoveGoalHandle> goal_handle)
{
  ...
  // TODO: Should this be more deterministically linked to the execution thread?
  //       Maybe low-ish (lower than execution thread) prio faster wait check?
  double rate_hz = 1 / (_network_cycle_time_us / 1000000);
  rclcpp::Rate loop_rate(rate_hz);

  const auto goal = goal_handle->get_goal();
  const auto time_vec = goal->time_data;
  const auto value_vec = goal->value_data;

  auto feedback = std::make_shared<ExecuteMove::Feedback>();
  auto & time_fb = feedback->time_data;
  auto & value_fb = feedback->value_data;
  ...
  for (std::size_t i = 0; (i < time_vec.size()) && rclcpp::ok(); i++) {
    ...
    for (std::size_t i = 0; i < time_fb.size(); i++) {
      data += "(" + std::to_string(time_fb.at(i)) + ", " + std::to_string(value_fb.at(i)) + ")";
      if (i < goal->time_data.size() - 1) {
        data += "; ";
      }
    }
